...
REQ-001 的目标是将 Codex CLI/app-server 能力改造成可由外部客户端按单次请求调用的无状态服务入口。服务实例可横向扩展到几十至上百个节点；一次用户输入对应一次 Request；Turn 执行期间允许绑定 owner 实例；Request 结束后，后续请求必须能由任意兼容实例基于云端状态恢复同一 Thread 上下文并继续执行。若当前 Turn 执行过程中 owner 实例故障，首版不恢复该 Turn；owner 租约超时持久化为专门的 `owner_timed_out` 终态，writer lease 丢失或写入被拒绝持久化为 `lease_lost` 终态，其他主动取消、服务端中断或不可恢复错误分别持久化为 `cancelled`、`interrupted` 或 `failed` 终态。[需求来源]
...
| ID | 约束 | 处理方式 | 来源类型 |
| --- | --- | --- | --- |
| CON-001 | 服务实例不得依赖进程内长期业务状态完成跨请求正确性 | Request 入口从云端 Thread、State 和 ConfigSnapshot 重建运行上下文；终态后释放 request-scoped runtime | 需求来源 |
...
设计结论：采用新增 Request 外层资源的方案，首版定位为内部云端 wrapper API。Request 负责幂等、排队、取消、事件游标和终态查询；Turn 负责模型执行轮次语义，并在执行期持有 Turn 级 OwnerLease。该方案避免把 queued、cancelling、cancelled、owner_timed_out、lease_lost 等请求级状态强行折叠进现有 `TurnStatus`。[需求来源]
...
| ENT-006 | StateDbMetadata | CloudStateStore | 云端 DB | 承接现有 SQLite state_db 中 thread metadata、goal、memory mode、backfill state 等职责 | 需求来源 |
...
| ENT-012 | ReadOnlyExecEnvironment | exec-server | 不持久化 | 只读脚本执行环境，workspace、检索挂载和临时目录不可写 | 需求来源 |
...
flowchart TD
    ...
    L --> N["Build request-scoped ReadOnlyRuntime"]
    ...
    R -->|Yes| T["Reject with runtime_write_denied and terminate as lease_lost"]
    R -->|Read-only observation| U["Execute through read-only exec-server"]
    T --> V
    U --> V
    V --> W{"Cancel requested?"}
...
stateDiagram-v2
    ...
    queued --> turn_timed_out: request_timeout
    ...
    running --> owner_timed_out: turn_owner_lease_timeout
    running --> lease_lost: writer_lease_lost_or_write_denied
    running --> turn_timed_out: turn_timeout
...
| ST-003 | cancelling | 调用方主动取消或调度终止已发起；单纯 SSE 客户端断线不改变状态 | `InProgress` 或 `Interrupted` | 推断 |
| ST-004 | completed | Turn 正常完成，关键状态和事件已持久化 | `Completed` | 代码证据 |
| ST-005 | failed | 模型、环境、状态存储或内部错误导致失败 | `Failed` | 代码证据 |
| ST-006 | cancelled | 调用方主动取消并确认 | `Interrupted` 或新增 `Cancelled` 投影 | 需求来源 |
| ST-007 | interrupted | 服务端执行被中断、owner 实例退出或临时运行态丢失 | `Interrupted` | 需求来源 |
| ST-008 | owner_timed_out | OwnerLease 超过 3 分钟 TTL 未续期；当前 Turn 不恢复 | 无或 `Failed` 投影 | 需求来源 |
| ST-009 | lease_lost | writer lease 丢失或 `runtime/writeDenied`，当前实例不再是合法 writer | 无或 `Failed` 投影 | 需求来源 |
| ST-010 | turn_timed_out | Turn 业务执行超过 `turn_timeout_ms` | 无或 `Failed` 投影 | 推断 |
...
OwnerLease 结论：OwnerLease 是 Turn 级执行租约，不是 Request 恢复租约。OwnerLease 最多等待 3 分钟，owner 实例每 2 秒续期一次；租约超时后调度器必须将当前 Request/Turn 标记为专门的 `owner_timed_out` 终态，主动退出或不可恢复错误可标记为 interrupted 或 failed，并释放 ThreadWriterLease 以便后续 queued Request 继续执行；不得恢复当前 Turn 或跨实例接管正在运行的模型、工具或 SSE 流。[需求来源]
...
| CA-017 | `codex-rs/exec/src/lib.rs` | `codex exec` 使用 in-process app-server client 执行非交互 turn | 可作为 single-run 入口，但不能替代云端状态、lease 和只读 Runtime | 代码证据 |
...
| CH-107 | `codex-rs/app-server/src/thread_state.rs` 和 watch manager | SSE 客户端断线只关闭当前流，不改变 running Request 状态；客户端主动 cancel 才路由 owner interrupt；首版不提供运行中重新 attach，只支持断线后按事件游标读取已持久化事件 | FR-005, FR-014 | 需求来源 |
| CH-108 | `codex-rs/app-server/README.md` | 更新 v2 API、只读 Runtime、禁用文件修改能力和兼容约束 | FR-001 至 FR-009、FR-012 至 FR-015 | 代码规则 |
...
| CH-202 | 新 crate `codex-cloud-state` | 只实现云端 MySQL 状态：Request、Event、Turn 级 OwnerLease、MySQL ThreadWriterLease、fencing token、StateDbMetadata | FR-001 至 FR-015 | 需求来源 |
...
| ID | 文件或模块 | 变更 | 关联功能点 | 来源类型 |
| --- | --- | --- | --- | --- |
| CH-301 | `codex-rs/exec/src/cli.rs` | 不新增 `--single-run`、`--one-shot` 或 `--run-once`；直接复用现有 `codex exec` 作为 single-run/non-interactive 入口 | FR-009 | 需求来源 |
...
| DATA-006 | `OwnerLeaseRecord` | `turn_id`、`request_id`、`owner_instance_id`、`heartbeat_at`、`expires_at`、`cancel_requested_at` | Turn 执行期租约；owner 失联后当前 Turn 不恢复，Request 进入 `owner_timed_out`、`interrupted` 或 `failed` | 需求来源 |
...
| IF-003 | wrapper `request/events/list` | `requestId`、`cursor?`、`limit?` | `data`、`nextCursor` | 按游标读取已持久化事件；SSE 客户端断线不改变 Request 状态，客户端可用该接口补读事件 | 需求来源 |
...
云端状态存储首版使用 MySQL。MySQL 承载 Request、Thread、Turn、Event、ThreadWriterLease、OwnerLease、fencing token、StateDbMetadata 和 runtime write-denied metadata。云端对象存储仅承载超过 MySQL 行内阈值的大事件 payload。EventRecord 首版使用 MySQL `events` 表按 `(request_id, sequence)` 追加；如后续吞吐要求超过 MySQL 写入能力，再评估独立事件日志。[需求来源]

| ID | 表或集合 | 主键 | 唯一约束 | 重要索引 | 来源类型 |
| --- | --- | --- | --- | --- | --- |
| DB-001 | `threads` | `thread_id` | 无 | `updated_at`、`caller_id`、`archived_at` | 需求来源 |
| DB-002 | `thread_items` | `item_id` | `(thread_id, append_sequence)`、`(thread_id, append_idempotency_key)` | `thread_id, append_sequence` | 推断 |
...
| MSG-005 | `request/failed` | server notification | `requestId`、`errorKind`、`errorCode`、`finalCursor` | failed/interrupted/owner_timed_out/lease_lost/turn_timed_out 诊断 | 推断 |
...
| FR-008 严格只读 Runtime | CH-104, CH-105, CH-205, CH-206 | IF-006 | ST-005 或 ST-009 | DATA-010, MSG-006 | 需求来源 |
| FR-009 CLI 兼容入口 | CH-301, CH-302, CH-303 | IF-001, IF-009 | ST-001 至 ST-008 | DATA-001 | 需求来源 |
| FR-012 结构化错误 | CH-101, CH-202 | IF-002, IF-003 | ST-005 至 ST-008 | MSG-005, MSG-006 | 需求来源 |
| FR-013 文件修改能力禁用 | CH-002, CH-205, CH-209 | IF-005 | ST-002 或 ST-005 | DATA-010, DB-012 | 需求来源 |
| FR-014 SSE 断线与补读 | CH-101, CH-107 | IF-003, IF-004 | ST-002；主动 cancel 才进入 ST-003/ST-006 | DATA-001, MSG-002 | 需求来源 |
...
| --- | --- | --- | --- | --- |
| TC-001 | app-server protocol serialization | Request、Runtime DTO JSON/TS schema 正确；不生成文件修改产物 DTO | `codex-rs/app-server-protocol/src/protocol/v2/tests.rs` | 代码规则 |
...
| TC-006 | SSE disconnect | running SSE 客户端断线不改变 Request 状态；后续可按 cursor 补读已持久化事件；主动 cancel 才进入 cancelling/终态 | app-server transport/request tests | 需求来源 |
...
| CFG-002B | `cloud_runtime.mysql.max_connections` | `20` | MySQL 连接池上限；首版按单实例保守值，后续按 QPS、实例数和 MySQL 规格调优 | 需求来源 |
...
| CFG-005 | `cloud_runtime.turn_owner_lease_ttl_ms` | `180000` | Turn owner 租约 TTL；最多等待 3 分钟，只影响 owner lost 判定窗口，不定义恢复窗口 | 需求来源 |
| CFG-005A | `cloud_runtime.turn_owner_lease_heartbeat_interval_ms` | `2000` | Turn owner 心跳续期间隔；owner 实例每 2 秒续期一次 | 需求来源 |
| CFG-006 | `cloud_runtime.thread_writer_lease_ttl_ms` | `30000` | 同 Thread MySQL 写入租约 TTL；由 owner 周期续期，续期失败后进入 `lease_lost` 终止流程 | 需求来源 |
| CFG-006A | `cloud_runtime.thread_writer_lease_renew_interval_ms` | `2000` | MySQL 写入租约续期间隔；续期失败后当前实例必须停止写入并进入 `lease_lost` 终止流程 | 需求来源 |
| CFG-007 | `cloud_runtime.request_timeout_ms` | `60000` | 单次外部请求超时，例如模型请求、CloudThreadStore 请求等，默认 60 秒 | 需求来源 |
| CFG-007A | `cloud_runtime.turn_timeout_ms` | `1800000` | Request/Turn 业务执行总超时，默认 30 分钟；超时进入 `turn_timed_out` | 需求来源 |
| CFG-008 | `cloud_runtime.sse_disconnect_policy` | `keep_running` | running SSE 客户端断线不改变 Request 状态；客户端主动取消才进入取消流程 | 需求来源 |
...
| CFG-017 | `cli.single_run_mode` | `codex exec` | 非交互单次执行模式复用现有 `codex exec`；不新增 single-run 参数名 | 需求来源 |

外部依赖如下：

| ID | 依赖 | 用途 | 失败语义 | 来源类型 |
| --- | --- | --- | --- | --- |
| DEP-001 | MySQL | Request、Thread、Turn、Event、ThreadWriterLease、OwnerLease、fencing token、StateDbMetadata、runtime write-denied metadata | 创建、租约获取、续期或终态写入失败时不得确认 completed | 需求来源 |
...
| --- | --- | --- | --- | --- | --- | --- |
| DDR-001 | 需求来源 | 单次请求接口是否复用 `turn/start` | A 扩展 `turn/start`；B 新增内部 wrapper `request/run`；C 公开 app-server experimental v2 `request/run` | 选择 B | Request 需要 queued、cancelling、cancelled、owner_timed_out、lease_lost、幂等和事件游标；现有 TurnStatus 不足。首版作为内部云端 wrapper，可降低公开 API 兼容承诺 | 新增内部协议面；后续若公开到 app-server v2 需要补 schema、README 和客户端兼容策略 |
...
| DDR-006 | 需求来源 | running SSE 断线是否改变 Request 状态 | A 允许运行中重新 attach；B 断线即取消或中断；C 断线不改变状态，只允许按事件游标补读 | 选择 C | SSE 是客户端观察通道，不应把浏览器刷新或网络抖动等同于服务端执行中断；首版不支持运行中重新 attach，但可通过 `request/events/list` 补读已持久化事件 | 客户端断线期间不能实时观察，重连后只能补读已持久化事件 |
| DDR-007 | 代码规则 | 云端状态实现放置位置 | A 加入 `codex-core`；B 新 crate 或现有非 core crate | 选择 B | 避免扩大 codex-core，符合项目规则 | trait 和 crate 边界需要设计 |
| DDR-008 | 需求来源 | CLI single-run 入口 | A 复用 `-p`；B 新增 `--single-run`/`--one-shot`/`--run-once`；C 复用现有 `codex exec` | 选择 C | Codex 已有 non-interactive/headless 模式 `codex exec`，不需要新增参数名，也避免与 `--profile` 等现有参数冲突 | 无状态服务专用行为只能在 `codex exec` 现有体系下扩展配置或子选项 |
| DDR-009 | 需求来源 | 同 Thread 并发请求如何处理 | A 排队；B 直接返回 busy/conflict；C 允许并发执行 | 选择 A | 同一 Thread 上下文需要串行追加 canonical items；排队能保留客户端提交语义并避免调用方自行重试 | 需要队列超时、队列长度限制和调度策略 |
...
| DDR-011 | 需求来源 | `OwnerLease` 的作用域和故障语义 | A Request 级 owner lease 并支持恢复当前 Turn；B Turn 级 owner lease，故障后不恢复当前 Turn；C 不设置 owner lease | 选择 B | 当前 Turn 运行态包含模型连接、工具调用和 SSE 流，首版不支持运行中跨实例接管；Turn 级租约足以支持取消路由、心跳和故障诊断；租约 TTL 为 3 分钟，owner 每 2 秒续期，超时后进入专门的 `owner_timed_out` 终态 | owner 故障会导致当前 Turn 终止；后续 queued Request 只能在 ThreadWriterLease 释放后继续 |
...
| CI-003 | Turn owner 心跳延迟 | 心跳未过期时非 owner 不得接管 running Turn；3 分钟租约过期后只允许将当前 Request/Turn 标记为专门的 `owner_timed_out` 终态，不恢复执行 | 需求来源 |
| CI-004 | append 重试 | 使用 append idempotency key 去重；重复 append 返回成功但不重复写 item | 推断 |
| CI-005 | fencing 冲突 | 非当前 fencing token、非当前 lease_id、已过期 lease 或已释放 lease 的 append 和 terminal 写入必须失败 | 需求来源 |
...
| ERR-007 | runtime_write_denied | 当前 turn 直接终止并进入 `lease_lost`；不得继续执行可能产生副作用的工具或写入 | 事件记录 `runtime/writeDenied` 和 `lease/lost` | 需求来源 |
| ERR-010 | owner_lost | `owner_timed_out`、interrupted 或 failed | 当前 Turn 不恢复；终态可查询 | 需求来源 |
...
| ID | 安全面 | 要求 | 来源类型 |
| --- | --- | --- | --- |
| SEC-001 | 执行环境写入 | App-Server 主机、exec-server workspace、检索挂载和执行临时目录不可写 | 需求来源 |
...
安全结论：只读 Runtime 是系统级 capability profile，不是单个 sandbox policy。必须在 app-server 协议入口、core tool registry、exec-server、MCP、hook 和 patch runtime 同时执行。[推断]

## 11. 方案补强与默认解决方案

本节补充待确认项之外的实现细节。除明确标为“需要再确认”的事项外，均作为首版默认方案进入实现设计。[需求来源]

### 11.1 状态机统一规则

Request/Turn 终态统一使用以下语义，客户端展示和统计不得再混用：

| 终态 | 触发条件 | 是否用户主动 | 是否允许 retry | 默认展示 |
| --- | --- | --- | --- | --- |
| `completed` | 模型执行完成，事件、Thread item 和终态已持久化 | 否 | 不需要 | 已完成 |
| `cancelled` | 调用方主动取消，且 owner 已确认中断或尚未开始执行 | 是 | 可由用户重新发起新 Request | 已取消 |
| `interrupted` | 服务端进程退出、运行态不可恢复、非 lease 类系统中断 | 否 | 可由用户重新发起新 Request | 已中断 |
| `owner_timed_out` | OwnerLease 超过 3 分钟 TTL 未续期 | 否 | 可由用户重新发起新 Request | 执行实例超时 |
| `lease_lost` | ThreadWriterLease 丢失、续期失败或 `runtime/writeDenied` | 否 | 可由用户重新发起新 Request | 写入租约丢失 |
| `turn_timed_out` | Request/Turn 执行超过 `turn_timeout_ms`，默认 30 分钟 | 否 | 可由用户重新发起新 Request | 执行超时 |
| `failed` | 模型、状态存储、exec-server 等不可恢复错误 | 否 | 视错误类型决定 | 执行失败 |

状态优先级默认如下：

1. `completed` 已持久化后，任何取消、lease 超时或断线事件都不得覆盖完成态。
2. `lease_lost` 优先于普通 `failed`，因为它代表当前实例已不再具备合法写入资格。
3. `owner_timed_out` 优先于 `interrupted`，因为它有明确的租约超时根因。
4. `turn_timed_out` 优先于普通 `failed`，但低于已持久化的 `completed`。
5. 客户端 SSE 断线不参与状态优先级；它只影响观察通道，不改变 Request 状态。

### 11.2 Lease、超时和终止职责

默认由云端调度器或 CloudStateStore 后台扫描任务负责检测过期租约和超时 Request。owner 实例正常运行时也可以主动检查并写入终态，但所有终态写入都必须经过 MySQL 条件更新和 fencing 校验。[推断]

| 场景 | 默认处理 |
| --- | --- |
| OwnerLease 心跳失败 | owner 继续按 2 秒间隔重试；只要未超过 3 分钟 TTL，不判定 lost |
| OwnerLease 超过 TTL | 调度器将 Request/Turn 置为 `owner_timed_out`，释放 ThreadWriterLease |
| ThreadWriterLease 续期失败 | 当前实例立即停止写入和工具执行，写入 `lease_lost` 诊断；若终态写入也因 fencing 失败，则只记录本地日志，由调度器兜底 |
| Request/Turn 超过 30 分钟 | 调度器写入 `turn_timed_out`，并请求 owner 停止执行；owner 不响应时仍按终态处理 |
| owner timeout 和 turn timeout 同时发生 | 优先记录 `owner_timed_out`，因为它解释了无法继续执行的直接原因 |
| writer lease 丢失后 owner lease | owner lease 视为失效，必须停止续期；ThreadWriterLease 释放或过期后才允许下一 Request 运行 |

### 11.3 幂等、去重和重复执行控制

首版默认将“重复写入可去重”和“重复副作用不可自动重放”分开处理：

- Request 创建必须携带 `idempotencyKey`；唯一键为 `(caller_id, thread_id, idempotency_key)`。
- 幂等键命中且 input hash 相同，返回原 Request；input hash 不同，返回 idempotency conflict。
- 每次 `thread/appendWithLease` 必须携带 `append_idempotency_key`，唯一键为 `(thread_id, append_idempotency_key)`。
- Thread item sequence 由 MySQL 在当前 writer lease/fencing token 校验通过后分配或确认。
- 同批 append 多条 item 必须原子写入；部分成功不可见。
- tool call / exec 默认不自动重放。若 Request 因 `owner_timed_out`、`lease_lost`、`interrupted` 或 `turn_timed_out` 终止，用户 retry 必须创建新 Request，并在上下文中展示旧 Request 的已持久化输出和终态原因。
- 旧 owner 在租约过期后继续执行的结果不得写入 canonical Thread；即使本地产生输出，也只能作为本地日志丢弃。

### 11.4 `thread/appendWithLease` 默认接口契约

`AppendThreadItemsParams` 保持兼容，不直接扩展语义。云端新增 lease-aware append API，默认内部方法名为 `thread/appendWithLease`；如果后续进入 Rust trait，可命名为 `append_thread_items_with_lease`。[需求来源]

默认请求字段：

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `threadId` | 是 | 目标 Thread |
| `requestId` | 是 | 当前 Request |
| `leaseId` | 是 | 当前 ThreadWriterLease |
| `writerOwnerToken` | 是 | 随机 owner token，用于防止旧实例续写 |
| `fencingToken` | 是 | MySQL 单调递增 fencing token |
| `appendIdempotencyKey` | 是 | 同一批 append 的幂等键 |
| `expectedThreadVersion` | 否 | 调用方已知 Thread version；提供时必须匹配 |
| `items` | 是 | 要追加的 canonical items |

默认响应字段：

| 字段 | 说明 |
| --- | --- |
| `threadVersion` | 写入后的 Thread version |
| `firstAppendSequence` / `lastAppendSequence` | 本批 item 的 sequence 范围 |
| `deduplicated` | 是否命中 append 幂等去重 |

错误语义：

- `lease_lost`：leaseId、owner token、fencing token 任一校验失败，当前 Request 必须终止。
- `version_conflict`：`expectedThreadVersion` 不匹配；调用方可重新读取 Thread 后决定是否重试。
- `idempotency_conflict`：同一 `appendIdempotencyKey` 对应内容 hash 不一致。
- `deadlock_retry_exhausted`：MySQL 死锁按 50ms / 100ms / 200ms 重试 3 次仍失败。

### 11.5 MySQL 全新安装表结构默认方案
...
| 表 | 默认职责 |
| --- | --- |
| `threads` | Thread 元数据、caller/tenant 范围、版本号、归档状态、创建/更新时间 |
| `thread_items` | canonical Thread item，按 `(thread_id, append_sequence)` 有序追加，记录 append 幂等键和 payload ref |
| `requests` | Request 幂等、状态、owner、输入摘要、最新事件游标和超时字段 |
| `turns` | Turn 投影、状态、错误类型、开始/完成时间 |
| `events` | Request 范围事件日志，唯一键 `(request_id, sequence)` |
| `thread_writer_leases` | 同 Thread 写入互斥、owner token、leaseId、fencingToken、heartbeat/expires/released 时间 |
| `owner_leases` | Turn owner、heartbeat/expires、取消路由和失效诊断 |
| `config_snapshots` | Request 实际使用配置的 hash 和对象引用 |
| `state_metadata` | goal、memory mode、backfill 等云端 metadata |
| `runtime_write_denials` | 只读 Runtime 写入拒绝审计 |

默认 SQL 设计约束：

- 所有时间字段使用 UTC；API 边界如公开到 app-server v2 时转换为 Unix seconds。
- 大 payload 只在超过 MySQL 行内阈值时写对象存储，表内保存 `payload_ref`。
- 所有终态写入与事件终态写入必须在同一事务内完成。

### 11.6 失败恢复和用户 retry

首版不恢复正在运行的 Turn，不做跨实例接管。默认恢复策略如下：

- 用户 retry 总是创建新 Request，不复用旧 Request/Turn ID。
- retry 构造上下文时读取同一 Thread 的 canonical items；旧 Request 的未完成临时输出不进入 canonical Thread，除非已经成功 append。
- 前端/调用方展示旧 Request 的最终状态、错误类型和已持久化事件。
- `owner_timed_out` 展示为“执行实例超时”；`lease_lost` 展示为“写入租约丢失”；`turn_timed_out` 展示为“执行超时”。
- 如果需要自动 retry，必须由调用方显式配置重试策略；服务端首版不自动重放包含工具调用的 Request。

### 11.7 安全边界补充

- read-only exec-server 主文件系统保持只读，但允许每次 turn/exec 分配内存态 scratch。
- scratch 生命周期绑定 turn/exec，完成后必须清理；不得用于 Thread、Session 或 rollout path 持久化。
- scratch 默认按 caller/thread/request 隔离目录，目录名不得包含原始用户输入。
- scratch 默认需要容量上限和文件数上限；具体数值依赖部署规格，首版建议由部署系统配置，若未配置则使用 exec-server 默认限制。该项不阻塞方案实现。
- 云端 Runtime 禁止网络策略是否开放由 read-only exec-server 环境统一控制；Codex wrapper 不单独放宽网络权限。

### 11.8 仍需后续确认的问题

当前没有阻塞首版技术方案的待确认项。以下只作为实现阶段可选确认点，不影响默认方案：

- scratch 容量和文件数上限的具体数值，由部署规格决定。
- 是否对某些只读 exec 任务允许更长 `turn_timeout_ms`，默认仍为 30 分钟。

## 12. 决策确认记录

| ID | 问题 | 决策 | 来源类型 |
| --- | --- | --- | --- |
| Q-001 | Turn 级 `OwnerLease` TTL、心跳周期和超时终态 | TTL 3 分钟；心跳 2 秒一次；超时终态为 `owner_timed_out` | 需求来源 |
| Q-002 | `ThreadWriterLease` 的 MySQL 条件更新 SQL、隔离级别和死锁重试策略 | TTL 30 秒；续期 2 秒；MySQL `READ COMMITTED`；随机 owner token；条件更新允许无 owner/自己/旧 owner 过期；死锁重试 3 次，50ms/100ms/200ms | 需求来源 |
| Q-003 | `AppendThreadItemsParams` 直接扩展还是新增 lease-aware append API | 选择新增 `thread/appendWithLease` / `append_thread_items_with_lease`；旧 API 保持兼容 | 需求来源 |
| Q-007 | read-only exec-server 临时目录是否允许内存态 scratch | 允许内存态 scratch；生命周期绑定 turn/exec；不得承载业务状态 | 需求来源 |
| Q-008 | `runtime/writeDenied` 是否继续 turn | 不继续；当前 turn 直接终止为 `lease_lost` | 需求来源 |
| Q-009 | SSE 断线终态用 `cancelled` 还是 `interrupted` | 客户端 SSE 断线本身不改变状态；客户端主动取消为 `cancelled`；服务端执行中断为 `interrupted` | 需求来源 |
| Q-010 | CLI single-run 参数名称 | 不新增参数；复用现有 `codex exec` | 需求来源 |
| Q-012 | 配置默认值 | `mysql.max_connections=20`、`owner_lease_ttl_ms=180000`、`owner_lease_heartbeat_ms=2000`、`turn_timeout_ms=1800000`、`request_timeout_ms=60000` | 需求来源 |
